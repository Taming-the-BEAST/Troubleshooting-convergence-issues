...
\begin{framed}
Open BEAUti and load in the \textbf{tutorial\_run1.xml} file by navigating to \mi{File\textrightarrow Load}.
\end{framed}

By navigating between the different tabs at the top of the application window, we can inspect the data and each element of the analysis. For example, in the \mi{Site Model} panel, we can see that we are fitting a GTR substitution model with no gamma rate heterogeneity \ref{fig:beauti_run1}.

\begin{figure}[!h]
\centering
\fbox{\includegraphics[scale=0.7]{figures/beauti_run1}}
\caption{\small The \mi{Site Model} panel in BEAUTi}
...
\begin{framed}
Open BEAST and choose \textbf{tutorial\_run1.xml} as the BEAST XML File. Then click \mi{Run}.
\end{framed}

\begin{figure}[!h]
\centering
\fbox{\includegraphics[scale=0.4]{figures/beast_input_run1}}
...
\begin{framed}
Open Tracer and navigate to \mi{File\textrightarrow Import Trace File}, then open \textbf{tutorial\_run1.log}
\end{framed}
...
By selecting a parameter in the left panel and then clicking on the \mi{Trace} tab, we can see how the MCMC explored parameter space \ref{fig:tracer_run1}. For the clockRate parameter for instance, we see that the chain quickly converged to a value of about 0.01 (the true value used to simulate the sequence data), but mixing was poor, hence the low ESS. 
...
\begin{framed}
Load the \textbf{tutorial\_run1.xml} back into BEAUti using \mi{File\textrightarrow Load}. Navigate to the MCMC panel and increase the chain length to 1 million \label{fig:beauti_run2}. You may also want to change the File Name for the log file to tutorial\_run2.log so we do not overwrite our previous results. When done, navigate \mi{File\textrightarrow Save As} and save as \textbf{tutorial\_run2.xml}.
\end{framed}
...
\begin{framed}
Load the \textbf{tutorial\_run3.xml} back into BEAUti and select \mi{View \textrightarrow Show Operators panel}. Click on the black triangle next to UpDown clockRate. In the menu that drops down, check Optimize. In the box to the right, change the weight on the UpDown operator from 0.0 to 3.0. In the MCMC panel, change the File Name for the log file to tutorial\_run3.log. When done, save as \textbf{tutorial\_run4.xml}.
\end{framed}
